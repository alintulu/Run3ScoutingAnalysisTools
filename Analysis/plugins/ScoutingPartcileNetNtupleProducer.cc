...
FatJetMatching<reco::Jet> ak8_match;

class ScoutingPartcileNetNtupleProducer :  public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit ScoutingPartcileNetNtupleProducer(const edm::ParameterSet &);
  ~ScoutingPartcileNetNtupleProducer() override;
...
ScoutingPartcileNetNtupleProducer::ScoutingPartcileNetNtupleProducer(const edm::ParameterSet &iConfig)
...
ScoutingPartcileNetNtupleProducer::~ScoutingPartcileNetNtupleProducer() {}

void ScoutingPartcileNetNtupleProducer::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("gen_candidates", edm::InputTag("prunedGenParticles"));
  ...
  desc.add<edm::InputTag>("msd_value_map", edm::InputTag(""));
  desc.add<edm::InputTag>("n2b1_value_map", edm::InputTag(""));
  descriptions.add("pfDeepBoostedJetTagInfos", desc);
}

void ScoutingPartcileNetNtupleProducer::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  // Input jets
  ...
  for (std::size_t jet_n = 0; jet_n < jets->size(); jet_n++) {
    ...
    auto ak8_label = ak8_match.flavorLabel(jet, *gencands_, 0.8);
    ...
    std::cout << jet_n << ". " <<  jet.pt() << std::endl;
    tree->Fill();
    clearVars();
  }
}

void ScoutingPartcileNetNtupleProducer::clearVars(){
...
void ScoutingPartcileNetNtupleProducer::beginJob() {
}

void ScoutingPartcileNetNtupleProducer::endJob() {
}

// define this as a plug-in
DEFINE_FWK_MODULE(ScoutingPartcileNetNtupleProducer);
